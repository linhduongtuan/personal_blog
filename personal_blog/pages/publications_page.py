...
@rio.page(
    name="publications",
    url_segment="publications-page",
)
class PublicationsPage(rio.Component):
    def _create_paper_card(self, pub: Publication) -> rio.Component:
        return rio.Card(
            content=rio.Column(
                rio.Markdown(
                    f"**{pub.authors}**. " f'"__{pub.title}__"',
                    margin_top=1,
                    margin_left=1.5,
                    margin_right=1.5,
                    margin_bottom=0.2,
    ...
    def build(self) -> rio.Component:
        ...
        for pub in PUBLICATIONS:
            if pub.title and pub.authors:  # Skip empty entries
                if pub.year not in pub_by_year:
                    pub_by_year[pub.year] = []
                pub_by_year[pub.year].append(pub)

        # Create components list
        components = [rio.Text("Publications", style="heading1")]

        # Add year headers and papers
        for year in sorted(pub_by_year.keys(), reverse=True):
            components.append(rio.Text(f"{year}", style="heading2"))
            for pub in pub_by_year[year]:
                components.append(self._create_paper_card(pub))  # type: ignore
...
PUBLICATIONS = [
    ...
    Publication(
        title="GvmR – A Novel LysR-Type Transcriptional Regulator Involved in Virulence and Primary and Secondary Metabolism of __Burkholderia pseudomallei__.",
    ...
    Publication(
        authors="Tran Quang Binh, Duong Tuan Linh, Le Thi Kim Chung, Pham Tran Phuong, Bui Thi Thuy Nga, Nguyen Anh Ngoc, Tran Quang Thuyen, Do Dinh Tung & Bui Thi Nhung",
